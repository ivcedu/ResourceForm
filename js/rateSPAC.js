...
function initializeTable() {
    ...
    .children('tbody').on('editComplete', 'td', function(event, config){
        ...
        current_id = current_id.replace("spa_rate_col_", "");
        var ar_id = current_id.split('_');
        var column = "spa_mbr_" + ar_id[0];
        var rate_spac_id = ar_id[1];
        
        var num_rating = ratingConvertValue(edit_value);
        if (user_browser === "Firefox") {
            setTimeout(function() {
                calculateMedianMean(rate_spac_id, column, num_rating);
            }, 100);
...
function setAdminOption() {
    ...
    if (login_email === "ykim160@ivc.edu" || login_email === "bhagan@ivc.edu") {
        ...
        if (result[0]['spaUserAdmin'] === "1") {
//            admin = true;
            $('#nav_admin_setting').show();
        }
        rate_column = result[0]['spaColumnName'];
    }
}
...
function getTableHeaderArray() {
    var result = new Array(); 
    result = db_getmbrSPAC(login_email, admin);
    ...
    for(var i = 0; i < result.length; i++) {
        ...
        ar_column.push(num_column, result[i]['spaUserName']);
...
function setTableHeader() {    
    ...
    tbl_html += "<th class='col_50' style='text-align: center;' id='spa_median'>Median</th>";
    tbl_html += "<th class='col_50' style='text-align: center;' id='spa_mean'>Mean</th>";
    
    for (var i = 0; i < ar_header.length; i++) {
        var num_column = ar_header[i][0];  
        var name_column = ar_header[i][1];
        tbl_html += "<th class='col_100' style='text-align: center;' id='spa_mbr_" + num_column + "'>" + name_column + "</th>";
...
function getrateSPACList() {
    var result = new Array();
    result = db_getrateSPACList();
    ...
    if (result.length !== 0) {
        for(var i = 0; i < result.length; i++) {
            ...
            getrateSPACListHTML(result[i]['rateSPAC_ID'], result[i]['ResourceID'], result[i]['ProposalTitle'], str_totalAmount, result[i]['ResourceType'], result[i]['CreatorName'], result[i]['StageLevel']);
            setMedianValue(result[i]['rateSPAC_ID'], result[i]['Median']);
            setMeanValue(result[i]['rateSPAC_ID'], result[i]['Mean']);
            setMemberRatingValue(result[i]['rateSPAC_ID'], i, result);
        }
    }
}

function getrateSPACListHTML(rate_spac_id, resource_id, title, amount, type, creator, stage) {
    var brief_title = textTruncate(30, title);
    
    var tbl_html = "<tr id='res_tr_" + rate_spac_id + "'>";
    ...
    tbl_html += "<td class='col_50' style='text-align: center;' id='spa_median_" + rate_spac_id + "'></td>";
    tbl_html += "<td class='col_50' style='text-align: center;' id='spa_mean_" + rate_spac_id + "'></td>";
    
    for (var i = 0; i < ar_header.length; i++) {
        var num_column = ar_header[i][0];  
        tbl_html += "<td class='col_100' style='text-align: center;' id='spa_rate_col_" + num_column + "_" + rate_spac_id + "'></td>";
    }
...
function setMedianValue(rate_spac_id, median) {
    if (median !== null) {
        $('#spa_median_' + rate_spac_id).html(median);
    }
}

function setMeanValue(rate_spac_id, mean) {
    if (mean !== null) {
        $('#spa_mean_' + rate_spac_id).html(mean);
    }
}

function setMemberRatingValue(rate_spac_id, index, result) {    
    for (var i = 0; i < ar_header.length; i++) {
        var num_column = ar_header[i][0];
        var column_name = "spa_mbr_" +  num_column;
        var mbr_value = result[index][column_name];
        if (mbr_value !== null) {
            $('#spa_rate_col_' + num_column + '_' + rate_spac_id).html(mbr_value);
        }
    }
}
...
function getMemberList() {
    ...
    if (result.length !== 0) {
        for(var i = 0; i < result.length; i++) {
            setMemberListHTML(result[i]['mbrSPAC_ID'], result[i]['spaColumnName']);
            setMemberNameHTML(result[i]['mbrSPAC_ID'], result[i]['spaUserName']);
            setMemberEmailHTML(result[i]['mbrSPAC_ID'], result[i]['spaUserEmail']);
            setMemberActiveHTML(result[i]['mbrSPAC_ID'], result[i]['spaUserActive']);
            setMemberAdminHTML(result[i]['mbrSPAC_ID'], result[i]['spaUserAdmin']);
        }
    }
}

function setMemberListHTML(mbr_spac_id, spa_column_name) {
    var tbl_html = "<tr class='row_tr' id='mbr_spac_id_" + mbr_spac_id + "'>";
    tbl_html += "<td class='span4'><input type='text' class='span12' id='mbr_user_name_" + mbr_spac_id + "'></td>";
    tbl_html += "<td class='span4'><input type='text' class='span12' id='mbr_user_email_" + mbr_spac_id + "'></td>";
    tbl_html += "<td class='span1 form-horizontal' style='text-align: center;'><input type='checkbox' id='mbr_user_active_" + mbr_spac_id + "'></td>";
    tbl_html += "<td class='span1 form-horizontal' style='text-align: center;'><input type='checkbox' id='mbr_user_admin_" + mbr_spac_id + "'></td>";
    tbl_html += "<td class='span1 form-horizontal'><button class='btn btn-mini span12' id='mbr_user_btn_update_" + mbr_spac_id + "'><i class='icon-circle-arrow-up icon-black'></i></button></td>";
    tbl_html += "<td class='span1 form-horizontal'><button class='btn btn-mini span12' id='mbr_user_btn_delete_" + mbr_spac_id + "'><i class='icon-trash icon-black'></i></button></td>";
    tbl_html += "<td class='span1' style='display: none;' id='mbr_column_name_" + mbr_spac_id + "'>" + spa_column_name + "</td>";
    tbl_html += "</tr>";
    
    $("#mod_body_tr").append(tbl_html);
}

function setMemberNameHTML(mbr_spac_id, user_name) {
    $('#mbr_user_name_' + mbr_spac_id).val(user_name);
}

function setMemberEmailHTML(mbr_spac_id, user_email) {
    $('#mbr_user_email_' + mbr_spac_id).val(user_email);
}

function setMemberActiveHTML(mbr_spac_id, user_active) {
    var ckb_value = (user_active === "1" ? true : false);
    $('#mbr_user_active_' + mbr_spac_id).prop('checked', ckb_value);
}

function setMemberAdminHTML(mbr_spac_id, user_admin) {
    var ckb_value = (user_admin === "1" ? true : false);
    $('#mbr_user_admin_' + mbr_spac_id).prop('checked', ckb_value);
}
...
function updateSelectedMember(mbr_spac_id) {
    var name = $('#mbr_user_name_' + mbr_spac_id).val();
    var email = $('#mbr_user_email_' + mbr_spac_id).val();
    var active = ($('#mbr_user_active_' + mbr_spac_id).is(':checked') ? true : false);
    var admin = ($('#mbr_user_admin_' + mbr_spac_id).is(':checked') ? true : false);
    
    db_updatembrSPAC(mbr_spac_id, textReplaceApostrophe(name), textReplaceApostrophe(email), active, admin);
}

function deleteSelectedMember(mbr_spac_id, column) {
...
function calculateMedianMean(rate_spac_id, db_column, db_rating) {    
    db_updaterateSPACRating(rate_spac_id, db_column, db_rating);
    
    getDBAllColumnArray();
    sqlRawScriptUpdateMedian(rate_spac_id);
    sqlRawScriptUpdateMean(rate_spac_id);
    
    var result = new Array();
    result = db_getrateSPAC(rate_spac_id);
    
    if (result[0]['Median'] !== null) {
        $('#spa_median_' + rate_spac_id).html(result[0]['Median']);
    }
    
    if (result[0]['Mean'] !== null) {
        $('#spa_mean_' + rate_spac_id).html(result[0]['Mean']);
    }
}
...
function nullUpdateMedian() {
    var sql_query = "UPDATE [IVCRESOURCES].[dbo].[rateSPAC] SET Median = null";
    db_script_update_rate(sql_query);
}

function directUpdateMedian() {
    var num_column = ar_db_column[0][0];
    var sql_query = "UPDATE [IVCRESOURCES].[dbo].[rateSPAC] SET Median = spa_mbr_" + num_column;
    db_script_update_rate(sql_query);
}

function sqlRawScriptUpdateMedian(rate_spac_id) {
    var sql_query = "CREATE TABLE #MEDIAN_TABLE(rateSPAC_ID int, value numeric(3, 2)) INSERT INTO #MEDIAN_TABLE SELECT rateSPAC_ID, CONVERT(numeric(3, 2), Value) ";
    sql_query += "FROM (SELECT * FROM [IVCRESOURCES].[dbo].[rateSPAC] WHERE rateSPAC_ID = " + rate_spac_id + ") pivotedDate UNPIVOT (Value FOR AgeRAnge IN (";
    var sql_variable = "";
    ...
    sql_query += "CREATE TABLE #MEDIAN_RESULT (rateSPAC_ID int, column_median numeric(3, 2)) ";
    sql_query += ";WITH Counts AS(SELECT rateSPAC_ID, c = COUNT(*) FROM #MEDIAN_TABLE GROUP BY rateSPAC_ID) ";
    sql_query += "INSERT INTO #MEDIAN_RESULT SELECT a.rateSPAC_ID, Median = AVG(0. + value) FROM Counts a ";
    sql_query += "CROSS APPLY(SELECT TOP (((a.c - 1) / 2) + (1 + (1 - a.c % 2))) b.value, r = ROW_NUMBER() OVER (ORDER BY b.value) FROM #MEDIAN_TABLE b WHERE a.rateSPAC_ID = b.rateSPAC_ID ORDER BY b.value ) p ";
    sql_query += "WHERE r BETWEEN ((a.c - 1) / 2) + 1 AND (((a.c - 1) / 2) + (1 + (1 - a.c % 2))) GROUP BY a.rateSPAC_ID; ";
    sql_query += "UPDATE [IVCRESOURCES].[dbo].[rateSPAC] SET Median = rst.column_median ";
    sql_query += "FROM #MEDIAN_RESULT AS rst INNER JOIN [IVCRESOURCES].[dbo].[rateSPAC] AS org ON rst.rateSPAC_ID = org.rateSPAC_ID ";
...
function sqlAllScriptUpdateMedian() {
    ...
    sql_query += "FROM [IVCRESOURCES].[dbo].[rateSPAC] AS org LEFT JOIN #MEDIAN_RESULT AS rst ON org.rateSPAC_ID = rst.rateSPAC_ID ";
...
function nullUpdateMean() {
    var sql_query = "UPDATE [IVCRESOURCES].[dbo].[rateSPAC] SET Mean = null";
    db_script_update_rate(sql_query);
}

function directUpdateMean() {
    var num_column = ar_db_column[0][0];
    var sql_query = "UPDATE [IVCRESOURCES].[dbo].[rateSPAC] SET Mean = spa_mbr_" + num_column;
    db_script_update_rate(sql_query);
}

function sqlRawScriptUpdateMean(rate_spac_id) {
    var sql_query = "";
    ...
    sql_query += "WHERE rateSPAC_ID = " + rate_spac_id;
    db_script_update_rate(sql_query);
}
