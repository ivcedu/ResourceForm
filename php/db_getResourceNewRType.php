<?php
    ...
    if (isset($_POST['ResourceID']))
    {
        ...
        $query = "SELECT cr.CreatorName, cr.CreatorTitle, cr.CreatorDepartment, rs.strDate, rs.FiscalYear, rs.ProposalTitle, rs.NeedFor, rt.ResourceType, rs.LoginID, rs.CreatorID, pg.ProgramType, rs.OneTime "
        ...
        if ($rstCount > 0)
        {
            foreach($data as $row)
            {
                $rows[$index] = array($row['CreatorName'], $row['CreatorTitle'], $row['CreatorDepartment'], $row['strDate'], $row['FiscalYear'], $row['ProposalTitle'], $row['NeedFor'], $row['ResourceType'], $row['LoginID'], $row['CreatorID'], $row['ProgramType'], $row['OneTime']);
                $index++;
            }
        }
